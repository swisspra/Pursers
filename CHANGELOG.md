...
- Home acceptance: concrete verifier-owned browser observer
  ...
  SHA from the installed deterministic extension manifest, the selected board
  from rendered Home UI, and host identity from the signed live AionUi listener
  before writing a capture, then replays only its own
