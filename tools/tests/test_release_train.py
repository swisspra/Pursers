...
def test_explicit_bump_rewrites_fixture_consumers_without_touching_disk(
    ...
    assert "5.0.0a25" in planned[root / "packages/pursers/pyproject.toml"]
    assert "pursers-central==0.1.0a29" in planned[
        root / "packages/personal/pyproject.toml"
    ]
    assert "pursers-client==0.1.0a22" in planned[
        root / "tools/wait-bridge/pyproject.toml"
    ]
    assert "pursers-client==0.1.0a22" in planned[
        root / "packages/central/pyproject.toml"
    ]
    assert 'SOURCE_VERSION = "0.1.0a15"' in planned[
        root / "tools/wait-bridge/pursers_wait_server.py"
    ]
    assert "## [5.0.0a25] - " in planned[root / "CHANGELOG.md"]
    assert "5.0.0a25" not in (root / "packages/pursers/pyproject.toml").read_text()


def test_next_patch_alpha_advances_every_component() -> None:
    current = load_versions(ROOT / "tools/release_versions.toml")
    target = release_train.bumped_versions(current, (), "patch-alpha")
    assert target.product == "5.0.0a25"
...
def test_check_detects_fixture_dependency_drift(tmp_path: Path) -> None:
    ...
    pyproject.write_text(
        pyproject.read_text().replace(
            "pursers-client==0.1.0a21",
            "pursers-client==0.1.0a20",
        )
    )

    errors = release_train.check(root, manifest)

    assert any("missing pursers-client==0.1.0a21" in error for error in errors)
...
def test_check_detects_central_client_dependency_drift(tmp_path: Path) -> None:
    ...
    assert any(
        "packages/central/pyproject.toml: missing pursers-client==0.1.0a21" in error
        for error in errors
    )
...
def test_check_detects_wait_bridge_source_constant_drift(tmp_path: Path) -> None:
    ...
    source.write_text(
        source.read_text().replace(
            'SOURCE_VERSION = "0.1.0a14"', 'SOURCE_VERSION = "0.1.0a13"'
        )
    )

    errors = release_train.check(root, manifest)

    assert any("SOURCE_VERSION '0.1.0a13' != '0.1.0a14'" in error for error in errors)
