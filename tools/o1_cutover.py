...
def run_rollback(config: CutoverConfig, operator_confirmed: bool) -> dict[str, Any]:
    ...
    allowed = {
        (swap.step_id, swap.identifier): swap.live_path.resolve(strict=False)
        for swap in config.swaps()
    }
    journal_problems: list[str] = []
    for step_id in steps:
        for target in recorded.get(step_id) or []:
            identifier = str(target.get("identifier") or "")
            expected = allowed.get((step_id, identifier))
            destination = Path(str(target.get("live") or "")).resolve(strict=False)
            if expected is None:
                journal_problems.append(
                    f"journal target {step_id}/{identifier or '<missing>'} is not configured")
            elif destination != expected:
                journal_problems.append(
                    f"journal live path for {step_id}/{identifier} differs from the configured target")
    if journal_problems:
        raise GateFailure("rollback", journal_problems)
