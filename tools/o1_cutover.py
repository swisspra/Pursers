...
def kid_coherence_finding(
    ...
    required = {kid for kid in required_kids if kid}
...
def copy_rollback_unit(
    ...
    for identifier, live in entries:
        ...
        destination = config.backup_root / relative
        if live.is_dir():
...
def backup_is_current(config: "CutoverConfig") -> list[Finding]:
    ...
    for swap in config.swaps():
        backup_path = config.backup_root / swap.identifier
...
def load_config(path: str | Path, repo_root: str | Path | None = None) -> CutoverConfig:
    ...
    for entry in _require(raw, "credentials", "config"):
        ...
        identifier = str(_require(entry, "identifier", "credential"))
    ...
    for entry in _require(raw, "seats", "config"):
        ...
        managed = tuple(str(item) for item in (entry.get("managed") or MANAGED_SEAT_FILES))
    ...
    for entry in (raw.get("configs") or []):
        ...
        identifier = str(_require(entry, "identifier", "config file"))
    ...
    config = CutoverConfig(
        ...
        extra_backup_paths=tuple(
            (str(item.get("identifier")), _path(item.get("path"), "extra_backup_paths"))
...
def validate_layout(config: CutoverConfig) -> None:
    """Fail closed on a staging root that could touch the repository or live state."""
    problems: list[str] = []
    staging = config.staging_root
    if not staging.is_absolute():
        problems.append("staging_root must be absolute")
    for label, root in (("repository", config.repo_root), ("live", config.live_root)):
        try:
            staging.relative_to(root)
            problems.append(f"staging_root must not live under the {label} root")
        except ValueError:
            pass
    if staging == config.backup_root:
        problems.append("backup_root must be a distinct directory inside staging")
    try:
        config.repo_root.relative_to(staging)
        problems.append("repository root must not live under staging_root")
    except ValueError:
        pass
    for identifier, live in config.rollback_unit():
        try:
...
def _toolkit_sheet_line(config: CutoverConfig, step_id: str) -> str:
    toolkit = config.repo_root / "tools" / "o1_cutover.py"
    command = TOOLKIT_COMMANDS.get(step_id, step_id)
    if command == "activate":
        return (f"python {toolkit} activate --config {config.path} --operator-confirmed "
                f"--evidence {config.evidence_dir / '<FRESH_DRY_RUN_EVIDENCE>.json'}")
    if command == "backup":
        return f"python {toolkit} backup --config {config.path}"
    return f"python {toolkit} {command} --config {config.path}"
...
def _operator_sheet_lines(config: CutoverConfig, step_id: str) -> list[str]:
    ...
    if step_id == "drain-work":
        return [
            ...
            f"python {config.repo_root / 'tools' / 'o1_cutover.py'} snapshot-boards "
            f"--config {config.path} --out {config.board_snapshot}",
        ]
    ...
    if step_id == "commit-target-memberships":
        return [
            ...
            f"python {config.repo_root / 'tools' / 'o1_cutover.py'} snapshot-memberships "
            f"--config {config.path} --out {config.membership_snapshot or staging / 'membership-snapshot.json'}",
        ]
...
def rollback_coherence_findings(config: CutoverConfig) -> list[Finding]:
    """Refuse a mixed restore: old tokens must pair with the old JWKS only."""
    findings: list[Finding] = []
    kids: list[str | None] = []
    for credential in config.credentials:
        if not is_regular_file(credential.live):
            findings.append(Finding("rollback_coherence", False,
                                    f"restored credential {credential.identifier} is absent", {}))
            continue
        text = credential.live.read_text(encoding="utf-8", errors="replace")
        findings.extend(Finding("rollback_coherence", finding.ok, finding.detail,
                                finding.evidence)
                        for finding in credential_binding_findings(
                            f"restored-{credential.identifier}", text, config.previous_url))
        kids.append(token_kid(text, credential.identifier))
    if not is_regular_file(config.jwks_live):
        findings.append(Finding("rollback_coherence", False, "restored JWKS is absent", {}))
        return findings
    try:
        available = jwks_kids(read_json(config.jwks_live, "restored JWKS"))
    except (GateFailure, ConfigError) as exc:
        findings.append(Finding("rollback_coherence", False,
                                f"restored JWKS is unusable: {exc}", {}))
        return findings
    findings.append(kid_coherence_finding(
        "rollback_coherence", "restored credential set", kids, available))
    return findings
...
def run_rollback(config: CutoverConfig, operator_confirmed: bool) -> dict[str, Any]:
    ...
    for entry in journal:
        if entry.state in ("started", "done"):
            # The terminal entry for a step carries the authoritative target list;
            # overwriting avoids restoring the same identifier twice.
            recorded[entry.step_id] = list(entry.targets)
    ...
    for step_id in steps:
        ...
        for target in targets:
            ...
            source = config.backup_root / identifier
            ...
            expected = str(target.get("pre_sha256") or "")
            mode = int(target.get("mode") or PRIVATE_FILE_MODE)
            destination.parent.mkdir(parents=True, exist_ok=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "command-sheet":
            out = args.out or (config.evidence_dir / f"{args.phase}-command-sheet.txt")
            ensure_private_dir(out.parent)
