...
const ERROR_COPY = {
  ...
  conflict: 'Seat groups changed in another session. Refresh before retrying.',
  backend_unavailable: 'The board connection is unavailable. Reconnect the helper, then retry.',
  invalid_input: 'Check the group name and selected seats, then retry.',
};
...
async function connectHelper(event) {
  ...
  await Promise.all([loadConnection(), loadTeamStatus(), loadGroups()]);
}
