...
function createStartupView(result) {
  if (!result || !result.ok) {
    if (result && result.error === 'bridge_not_installed') {
      return {
        state: 'bridge-missing', icon: '↓', title: 'Wait bridge missing',
        summary: result.install_hint || 'Install pursers-wait-bridge, then reload this page.', seats: [],
      };
    }
    return {
      state: 'unavailable', icon: '!', title: 'Status unavailable',
      summary: 'Pursers could not read wait-bridge status. Check the bridge, then reload this page.', seats: [],
    };
  }
  ...
  if (seats.length === 0) {
    return {
      state: 'empty', icon: '○', title: 'No connected seats',
      summary: 'No worker or reviewer seat is connected yet. Paste a door to get started.', seats,
    };
  }
  return {
    state: 'ready', icon: '✓',
    title: `${seats.length} connected seat${seats.length === 1 ? '' : 's'}`,
    summary: `Wait bridge status is available. Push mode: ${valueOrDash(result.push_mode)}.`, seats,
  };
}
...
function connectionState(result) {
  ...
  if (value.outcome === 'partial' || value.connected || value.joined) {
    ...
    return {
      title: 'Connected; registration incomplete', central: 'Connected', board: 'Joined',
  ...
  if (value.ok && value.status) {
    return {
      title: 'Connected and registered', central: 'Connected', board: 'Joined',
      helper: 'Registered', message: '', recovery: null,
    };
  }
  if (['bridge_rejected', 'identity_conflict', 'rotation_required'].includes(code)) {
    return {
      title: 'Board join refused', central: 'Reached', board: 'Join refused', helper: 'Not registered',
  ...
  if (code === 'bridge_not_installed' || code === 'bridge_status_failed') {
    return {
      title: 'Local helper unavailable', central: 'Not checked', board: 'Not joined',
      helper: code === 'bridge_not_installed' ? 'Not installed' : 'Unavailable',
      message: value.install_hint || value.message || 'Restore pursers-wait-bridge, then try again.', recovery: null,
    };
  }
  if (code === 'server_unreachable') {
    return {
      title: 'Central unavailable', central: 'Unavailable', board: 'Not joined', helper: 'Not registered',
      message: value.message || 'Check the Central connection, then try again.', recovery: null,
    };
  }
  return null;
}

async function compatReadJson(response) {
  try { return await response.json(); } catch (_error) { return { ok: false, error: 'invalid_response' }; }
}

function mount(documentRef, fetchImpl) {
  ...
  const ui = {
    card: documentRef.querySelector('#status-card'), icon: documentRef.querySelector('#status-icon'),
    title: documentRef.querySelector('#status-title'), summary: documentRef.querySelector('#status-summary'),
  ...
  const showResult = (result) => renderStartupView(createStartupView(result), ui, documentRef);
  const showStatus = (status, pushMode) => showResult({
    ok: true, push_mode: status.push_mode || pushMode, seats: [status],
  });
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const result = await compatReadJson(response);
    ...
    if (!response.ok || !result.ok) {
      ...
      message.textContent = (state && state.message) || result.install_hint || 'Join failed.';
      return;
    }
    showConnection(state);
  ...
  recoverButton.addEventListener('click', async () => {
    ...
    try {
      const response = await fetchImpl('/pursers/onboarding/recover', {
        method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(recoveryTarget),
      });
      const result = await compatReadJson(response);
      showConnection(connectionState(result));
      if (result.status) showStatus(result.status);
      message.textContent = response.ok && result.ok
        ? 'Recovered and registered without re-entering the door.' : 'Recovery failed.';
    } finally { recoverButton.disabled = false; }
  });
  fetchImpl('/pursers/status').then(compatReadJson).then((result) => {
    showResult(result);
    if (!result.ok) showConnection(connectionState(result));
  }).catch(() => showResult({ ok: false, error: 'status_unavailable' }));
}

if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    connectionState, createStartupView, mount, normalizeSeat, recoveryPayload, renderStartupView,
  };
}
