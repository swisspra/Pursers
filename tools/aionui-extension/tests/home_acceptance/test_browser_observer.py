...
def test_ego_binding_reads_are_isolated_from_page_monkeypatches() -> None:
    """A page-owned fetch/querySelector override cannot supply either binding."""
    ...
    evaluations = script.split("Runtime.evaluate", 1)[1].split(
        "Runtime.evaluate", 1
    )
    ...
    assert "document.querySelector" in evaluations[1]
