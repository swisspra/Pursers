...
def _write_backend(tmp_path: Path, identifier: str) -> Path:
...
def test_non_contract_status_route_blocks_capture(tmp_path: Path) -> None:
    backend = _write_backend(tmp_path, "door_connect")
    observer_dir = _install(tmp_path, backend)
    with _status_host(b"<!doctype html><html></html>", content_type="text/html") as base_url:
        exit_code = runner_module.main(
            [
                "runner.py", "capture",
                "--observer", str(observer_dir),
