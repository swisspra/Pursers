...
class VerifierBrowserObserver:
    ...
    def capture(self, request: BrowserObservationRequest) -> TrustedBrowserCapture:
        ...
        expected = {
            ...
            "host_version", "host_build", "candidate_commit", "captured_at",
...
def discover_repository_capabilities(root: Path = EXTENSION_ROOT) -> RepositoryCapabilities:
    ...
    webui_contributions = webui if isinstance(webui, list) else []
    routes = tuple(
        sorted(
            {
                route["path"]
                for contribution in webui_contributions
                if isinstance(contribution, dict)
                for route in contribution.get("routes", [])
