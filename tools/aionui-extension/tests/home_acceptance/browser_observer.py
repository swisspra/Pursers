...
def probe_host_identity(base_url: str, timeout_s: float = 4.0) -> dict[str, str]:
    """Read host product/version/build from the live host status route only."""
    opener = build_opener(ProxyHandler({}))
    endpoint = urljoin(f"{base_url}/", "pursers/status")
    try:
        ...
        raise _fail(
            ...
            f"host status route answered HTTP {exc.code}; AionUi host identity unavailable",
        ) from None
    except (URLError, TimeoutError, OSError) as exc:
        raise _fail(
            EXIT_CAPABILITY_UNAVAILABLE,
            f"host status route unreachable ({type(exc).__name__}); host identity unavailable",
        ) from None
    try:
        ...
        raise _fail(
            EXIT_CAPABILITY_UNAVAILABLE,
            f"host status route returned {content_type}, not the Pursers status contract",
        ) from None
...
EGO_SCRIPT = """
const target = %s
const task = await useOrCreateTaskSpace('pursers-home-acceptance')
...
def _run_backend(config: dict[str, Any], page_url: str) -> dict[str, Any]:
    ...
    if backend["kind"] == "ego-browser":
        argv = [*command, "nodejs"]
        payload = EGO_SCRIPT % json.dumps(page_url)
    ...
    for stream in (completed.stdout, completed.stderr):
        ...
        if observation is not None:
            break
    if observation is None or set(observation) != {"page_url", "screenshot_base64", "snapshot"}:
...
def capture(spec_path: Path, out: Any) -> int:
    ...
    host = probe_host_identity(spec["target"]["base_url"])
