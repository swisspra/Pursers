...
def _observed_runtime_binding(
    ...
    The authenticated same-origin status contract, read inside the verifier-created
    isolated world, is the primary binding. The signed live AionUi listener is an
    independent process-level identity proof used when that contract cannot be
    verified. The installed deterministic extension manifest is an independent
    candidate-SHA proof. When two sources are available they must agree before a
    capture is written.
    """
    ...
    if contract_ok:
        host = {
            "product": contract_host["product"],
            "version": contract_host["version"],
            "build": contract_host["build"],
            "source": "host-api",
        }
    else:
        try:
            ...
            raise _fail(
                ...
                "authenticated same-origin host status lacks a verifiable AionUi "
                "identity and the live AionUi listener is not verifiable",
            ) from None
    contract_extension = (
