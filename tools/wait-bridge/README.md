...
| AionUi 2.2.1 / nested Codex CLI over stdio | `{"form":{},"url":{}}` from the nested Codex client, not AionUi itself | **Push: no** on the tested fallback (`a2a_wait` returned `mode: "poll"`). **Native AionUi elicitation form: no**: a pending sandbox request produced only a generic `Elicitation` allow/reject confirmation, without the requested schema or disposition field. Extension MCPs were listed but not injected; an authenticated `/api/mcp/servers/import` plus API-created conversation loaded the bridge. See `docs/aionui-spike-2026-09.md`. |
