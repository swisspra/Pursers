...
class OrchestratorEngine:
    ...
    def __init__(
        ...
        self.known_active_boards: list[str] = [BOARD_ID]
    ...
    async def build_digest(
        ...
        try:
            client = await asyncio.wait_for(self._get_client(), timeout=0.2)
        except Exception:
            client = None

        if isinstance(boards, str):
            if boards == "registry":
                target_boards = list(self.known_active_boards or [BOARD_ID])
                if client is not None:
                    try:
                        reg = await asyncio.wait_for(_read_project_registry(client), timeout=0.2)
                        target_boards = _registry_boards(reg)
                        self.known_active_boards = list(target_boards)
                    except Exception:
                        pass
            else:
        ...
        for (bid, tid), evs in by_ticket.items():
            ...
            if ticket_data is None and client is not None:
                try:
                    view = _BoardView(client, bid)
                    t_resp = await asyncio.wait_for(view.ticket_get(tid), timeout=0.2)
                    if t_resp and "ticket" in t_resp:
