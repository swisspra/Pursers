...
class OrchestratorModeTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_dynamic_registry_activation_reopens_listen_and_receives_event(
        ...
        async with Client(self.mcp, mode="2026-07-28", cache=None) as raw_client:
            ...
            try:
                await asyncio.wait_for(engine.ready.wait(), timeout=3.0)
                self.assertIn(wait_server.BOARD_ID, engine.known_active_boards)
                self.assertNotIn(second_board, engine.known_active_boards)
                ...
                while time.monotonic() < deadline:
                    ...
                    if any(t["ticket_id"] == tid2 for t in d["tickets"]):
                        found = True
                        break
                    await asyncio.sleep(0.1)

                self.assertTrue(found, "Ticket from dynamically activated board was not found in digest")
    ...
    async def test_digest_prompt_return_with_delayed_or_unavailable_central(
        ...
        async with Client(self.mcp, mode="2026-07-28", cache=None) as raw_client:
            client, engine = await self._setup_client_and_engine(raw_client)
            engine.ticket_cache["pursers:TK-cached"] = {
                "ticket_id": "TK-cached",
                "title": "Cached Ticket",
                "status": "open",
            }
            ...
            engine.seen_event_ids.add("EV-cached")
            ...
            class SlowClient:
                ...
                async def ticket_get(self, *args, **kwargs):
                    await asyncio.sleep(5.0)
                    raise ConnectionError("delayed Central")

                async def board_state_get(self, *args, **kwargs):
                    await asyncio.sleep(5.0)
                    raise ConnectionError("delayed Central")

            slow = SlowClient(client)

            class SlowConn:
                async def client(self):
                    return slow

            engine.connection = SlowConn()

            t0 = time.monotonic()
            digest = await engine.build_digest(since=0)
            elapsed = time.monotonic() - t0
            self.assertLess(elapsed, 0.5)
            self.assertTrue(any(t["ticket_id"] == "TK-cached" for t in digest["tickets"]))
