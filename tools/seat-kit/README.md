...
commit and the current remote tip, then adds machine-derived preflight metadata
to the submitted notes. A malformed, nonexistent, stale, or mismatched SHA
fails before `ticket_submit`, so correct the evidence and retry; the ticket and
lease are not mutated. Explicit research-only tickets without that required
field retain the no-Git submit path.
