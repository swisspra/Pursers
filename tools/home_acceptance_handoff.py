...
def prepare(args: argparse.Namespace) -> dict[str, object]:
    ...
    package = _regular(_absolute(args.candidate_zip, "candidate ZIP"), "candidate ZIP")
    ...
    observer_runner = _regular(
        _absolute(args.observer_runner, "observer runner"), "observer runner"
    )
    ...
    observer_backend = _regular(
        _absolute(args.observer_backend, "observer backend"), "observer backend"
    )
    ...
    observer_harness = _regular(
        _absolute(args.observer_harness, "observer harness"), "observer harness"
    )
    ...
    helper = _regular(_absolute(args.helper, "helper"), "helper")
    if _sha256(helper) != args.helper_sha256:
        raise HandoffError("helper SHA-256 does not match approved runtime provenance")
    node = _regular(_absolute(args.node, "node"), "node", executable=True)
    bridge = _regular(_absolute(args.bridge_bin, "bridge binary"), "bridge binary", executable=True)
    aioncore = _regular(_absolute(args.aioncore_bin, "AionCore binary"), "AionCore binary", executable=True)
    ego_browser = _regular(
        _absolute(args.ego_browser, "ego-browser binary"), "ego-browser binary", executable=True
    )
    host_bundle = _absolute(args.host_bundle, "signed host bundle")
    codesign = _regular(_absolute(args.codesign, "codesign"), "codesign", executable=True)
    ...
    manifest = {
        ...
        "signed_host": {"bundle": str(host_bundle), "version": args.host_version, "cdhash": args.host_cdhash},
        "operator_authority": {
            "status": "required",
