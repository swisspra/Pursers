...
def test_inventory_and_doctor_redact_token_and_report_push(
    ...
    def run(command, **_kwargs):
        if command[0] == "ps":
            return subprocess.CompletedProcess(command, 0, "", "")
        return subprocess.CompletedProcess(command, 0, "0.1.0a8\n", "")
    ...
    inventory.upsert(target, bridge_version="0.1.0a8", doctor=report)
...
def test_doctor_poll_is_explicit_warning(
    ...
    def run(command, **_kwargs):
        stdout = "" if command[0] == "ps" else "0.1.0a8\n"
        return subprocess.CompletedProcess(command, 0, stdout, "")
...
def test_bridge_installer_stale_shim_and_pypi(tmp_path: Path) -> None:
    ...
    # 1. Stale shim: returns 0.1.0a6 while pinned is 0.1.0a8
    ...
    assert info["installed_version"] == "0.1.0a6"
    assert info["pinned_version"] == "0.1.0a8"
    assert info["latest_pypi_version"] == "0.1.0a8"
    assert info["status"] == "FAIL"
    assert "installed=0.1.0a6; pinned=0.1.0a8" in info["message"]
    ...
    assert info_warn["installed_version"] == "0.1.0a8"
