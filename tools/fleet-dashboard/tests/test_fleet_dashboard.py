...
def test_seat_config_manager_plan_apply_backup_restart_and_no_token_leak(
    ...
    assert bridge["installed_version"] == "0.1.1"
    assert bridge["reported_version"] == "0.1.0a1"
    assert bridge["pinned_version"] == "0.1.1"
    assert bridge["latest_pypi_version"] == "0.1.1"
...
def test_seat_config_manager_reviews_imports_and_doctors_discovered_seats(
    ...
    manager.inventory.upsert(conflicting, bridge_version="0.1.1")
...
def test_seat_config_registry_coverage_uses_live_fleet_seats(tmp_path: Path) -> None:
    ...
    manager.inventory.upsert(desired, bridge_version="0.1.1")
    ...
    class Seats:
        ...
        def bridge(self) -> dict:
            return {
                ...
                "package_metadata_version": "0.1.1",
                "pinned_version": "0.1.1",
                "latest_pypi_version": "0.1.1",
                "resolution_source": "config:codex",
                "status": "WARN",
                "message": (
                    "version string stale; reported=0.1.0a6; package=0.1.1"
                ),
            }
