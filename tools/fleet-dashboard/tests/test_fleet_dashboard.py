...
def test_reconnect_banner_keeps_cached_fleet_and_recovers_silently() -> None:
    ...
    program = "\n".join(
        [
            ...
            "const connectionFailures=new Set();let lastSuccessAt=null,connectionWasUnavailable=false;const announceState=()=>{};",
...
def test_help_theme_density_and_keyboard_controls_render() -> None:
    ...
    assert (
        'id="connection-banner" class="connection-banner" role="status" '
        'aria-live="polite" aria-atomic="true"' in dashboard.HTML
    )
    assert (
    ...
    assert "announceState(newest?'Dashboard updated'" in dashboard.HTML
