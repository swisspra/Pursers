...
def test_seat_config_manager_plan_apply_backup_restart_and_no_token_leak(
    ...
    manager = dashboard.SeatConfigManager(
        ...
        latest_version=lambda: "0.1.0a12",
    )
    desired = {
        "host": "codex",
        "role": "worker",
    ...
    assert bridge["installed_version"] == "0.1.0a12"
    assert bridge["reported_version"] == "0.1.0a1"
    assert bridge["pinned_version"] == "0.1.0a12"
    assert bridge["latest_pypi_version"] == "0.1.0a12"
...
def test_seat_config_manager_reviews_imports_and_doctors_discovered_seats(
    ...
    manager.inventory.upsert(conflicting, bridge_version="0.1.0a12")
...
def test_seat_config_registry_coverage_uses_live_fleet_seats(tmp_path: Path) -> None:
    ...
    manager.inventory.upsert(desired, bridge_version="0.1.0a12")
...
def test_config_api_and_ui_contract_are_separate_from_coordinator_config() -> None:
    ...
    class Seats:
        ...
        def bridge(self) -> dict:
            return {
                "installed_version": "0.1.0a12",
                "reported_version": "0.1.0a6",
                "package_metadata_version": "0.1.0a12",
                "pinned_version": "0.1.0a12",
                "latest_pypi_version": "0.1.0a12",
                "resolution_source": "config:codex",
                "status": "WARN",
                "message": (
                    "version string stale; reported=0.1.0a6; package=0.1.0a12"
                ),
            }
