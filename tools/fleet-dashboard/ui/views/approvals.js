...
(function registerApprovalsView() {
  ...
  let esc, fmt, pageHead, warmTruthStrip, warmTickets, ticketHref, warmBoards,
    boardHref, renderWaitingForYou, centralLabels, fleetData;
  ...
  function useContext(context) {
    ({esc, fmt, pageHead, warmTruthStrip, warmTickets, ticketHref, warmBoards,
      boardHref, renderWaitingForYou, centralLabels, fleetData} = context);
  }

  function sourceState() {
    const labels = Array.isArray(centralLabels) ? centralLabels : [];
    const connected = fleetData && typeof fleetData === 'object' ? fleetData : {};
    const missing = labels.filter(label => !connected[label]);
    if (!missing.length && (labels.length || Object.keys(connected).length)) return '';
    return `<aside class="approvals-source-state" role="status">
      <strong>Approval sources are not available yet.</strong>
      <span>Counts and empty states below cover connected sources only. Wait for the connection banner to clear before deciding that no action is needed.</span>
    </aside>`;
  }
