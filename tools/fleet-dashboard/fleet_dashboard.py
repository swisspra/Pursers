...
</style></head><body><main><div id="connection-banner" class="connection-banner" role="status" aria-live="polite" aria-atomic="true" hidden></div><div class="top"><div><h1>Fleet Dashboard</h1><p class="muted">Live boards and shared agent pool</p></div><div><div class="view-controls"><button id="theme-toggle" type="button">Light theme</button><button id="density-toggle" type="button">Compact density</button><button id="help-toggle" type="button" aria-label="Keyboard help">?</button></div><div class="search-wrap"><input id="filter" class="search" type="search" role="combobox" placeholder="Search boards, tickets, agents…" aria-label="Global dashboard search" autocomplete="off" aria-autocomplete="list" aria-haspopup="listbox" aria-controls="search-results" aria-expanded="false"><div id="search-results" class="search-results" role="listbox" aria-label="Global search results" hidden></div></div><div id="state" class="muted">Loading…</div><div id="live-state" class="sr-only" role="status" aria-live="polite" aria-atomic="true"></div></div></div><section id="home-view"><div id="central-sections"></div></section><section id="detail-view" hidden></section><section id="config-view" hidden></section><dialog id="help-overlay" aria-labelledby="help-title"><h2 id="help-title">Keyboard shortcuts</h2><dl class="shortcut-grid"><dt>/</dt><dd>Focus global search</dd><dt>g then f</dt><dd>Fleet overview</dd><dt>g then o</dt><dd>Protocol overhead</dd><dt>g then c</dt><dd>Coordinator config</dd><dt>↑ / ↓</dt><dd>Move within search results</dd><dt>Enter</dt><dd>Open the selected search result</dd><dt>?</dt><dd>Toggle this help</dd><dt>Esc</dt><dd>Close search or help</dd></dl><button id="help-close" type="button">Close</button></dialog></main><script>
...
function updateConnectionState(){const banner=document.querySelector('#connection-banner'),failure=[...connectionFailureDetails.values()].at(-1),failed=!!connectionFailures.size;banner.hidden=!failed;banner.textContent=failed?connectionBannerText(lastSuccessAt,failure):'';if(failed){connectionWasUnavailable=true;announceState('Connection interrupted. Reconnecting.')}else if(connectionWasUnavailable){connectionWasUnavailable=false;announceState('Connection restored.')}}
