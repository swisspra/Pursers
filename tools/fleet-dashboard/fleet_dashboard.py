...
function butlerHoldRows(){const rows=[];for(const [central,d] of Object.entries(fleetData)){for(const b of d.boards||[]){for(const f of b.coordinator_findings?.items||[]){const heldDraft=f.kind==='would_answer'&&['shadow','pending'].includes(f.hold?.status),heldAction=f.kind==='butler_action'&&f.hold?.status==='held';if(f.question_id&&(heldDraft||heldAction))rows.push({central,board:b,f})}}}return rows}
