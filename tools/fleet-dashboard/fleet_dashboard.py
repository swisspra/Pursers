...
const sectionStates=new Map(),connectionFailures=new Set();let lastAnnouncement='',connectionWasUnavailable=false;
...
function updateConnectionState(){const banner=document.querySelector('#connection-banner'),failed=!!connectionFailures.size;banner.hidden=!failed;banner.textContent=failed?connectionBannerText(lastSuccessAt):'';if(failed){connectionWasUnavailable=true;announceState('Connection interrupted. Reconnecting.')}else if(connectionWasUnavailable){connectionWasUnavailable=false;announceState('Connection restored.')}}
...
async function refreshConfig(){const r=route();if(!r||r.kind!=='config'||refreshPaused())return;const key=`config:${r.central}`;try{const response=await fetch(`/api/config?${apiCentral(r.central)}`,{cache:'no-store'});if(!response.ok)throw new Error(`HTTP ${response.status}`);const data=await response.json();if(route()?.central===r.central&&!refreshPaused()){renderConfig(data);markConnectionSuccess(key)}}catch(e){markConnectionFailure(key);if(!coordinatorConfig||coordinatorConfig.central!==r.central)document.querySelector('#config-view').innerHTML=`<a class="back" href="#/">← All centrals</a><p class="error">Config unavailable for ${esc(r.central)}.</p>`}}
...
async function refreshWorkers(force=false){const r=route();if(!r||r.kind!=='workers'||(workerFormDirty&&!force)||(!force&&refreshPaused()))return;const key=`workers:${r.central}`;try{const response=await fetch(`/api/workers?${apiCentral(r.central)}`,{cache:'no-store'}),body=await response.json();if(!response.ok)throw new Error(body.error||`HTTP ${response.status}`);if(route()?.central===r.central){renderWorkers(body);markConnectionSuccess(key)}}catch(e){markConnectionFailure(key);document.querySelector('#workers-view').innerHTML=`<a class="back" href="#/">← All centrals</a><p class="error">Workers unavailable for ${esc(r.central)}: ${esc(e.message)}</p>`}}
