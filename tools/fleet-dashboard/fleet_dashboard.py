...
</style></head><body><main><div id="connection-banner" class="connection-banner" role="status" hidden></div><div class="top"><div><h1>Fleet Dashboard</h1><p class="muted">Live boards and shared agent pool</p></div><div><div class="view-controls"><button id="theme-toggle" type="button">Light theme</button><button id="density-toggle" type="button">Compact density</button><button id="help-toggle" type="button" aria-label="Keyboard help">?</button></div><div class="search-wrap"><input id="filter" class="search" type="search" role="combobox" placeholder="Search boards, tickets, agents…" aria-label="Global dashboard search" autocomplete="off" aria-autocomplete="list" aria-haspopup="listbox" aria-controls="search-results" aria-expanded="false"><div id="search-results" class="search-results" role="listbox" aria-label="Global search results" hidden></div></div><div id="state" class="muted">Loading…</div></div></div><section id="home-view"><div id="central-sections"></div></section><section id="detail-view" hidden></section><section id="config-view" hidden></section><dialog id="help-overlay"><h2>Keyboard shortcuts</h2><dl class="shortcut-grid"><dt>/</dt><dd>Focus global search</dd><dt>g then f</dt><dd>Fleet overview</dd><dt>g then o</dt><dd>Protocol overhead</dd><dt>g then c</dt><dd>Coordinator config</dd><dt>↑ / ↓</dt><dd>Move within tables or search results</dd><dt>Enter</dt><dd>Open the selected search result</dd><dt>?</dt><dd>Toggle this help</dd><dt>Esc</dt><dd>Close search or help</dd></dl><button id="help-close" type="button">Close</button></dialog></main><script>
...
const sectionStates=new Map(),connectionFailures=new Set(),connectionFailureDetails=new Map();
...
function updateConnectionState(){const banner=document.querySelector('#connection-banner'),failure=[...connectionFailureDetails.values()].at(-1);banner.hidden=!connectionFailures.size;banner.textContent=connectionFailures.size?connectionBannerText(lastSuccessAt,failure):''}
