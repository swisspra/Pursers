...
function updateConnectionState(){const banner=document.querySelector('#connection-banner'),failed=!!connectionFailures.size;banner.hidden=!failed;banner.textContent=failed?connectionBannerText(lastSuccessAt):'';if(failed&&!connectionWasUnavailable){connectionWasUnavailable=true;announceState('Connection interrupted. Reconnecting.')}else if(!failed&&connectionWasUnavailable){connectionWasUnavailable=false;announceState('Connection restored.')}}
