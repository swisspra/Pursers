...
class ArchiveTierTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_memory_only_archive_sweep_updates_healthz_bytes_and_items(self) -> None:
        """Point 2: memory-only sweep persists bytes_freed and items_archived in healthz."""
        now = time.time()
        memory_id = "MEM-000001"

        def seed_mem(document: dict[str, object]) -> None:
            document["memories"] = [
                {
                    "memory_id": memory_id,
                    "title": "archived mem",
                    ...
                    "scope": "project",
                    "archived": True,
                    "archived_at": central.iso_at(now),
                }
            ]

        self.service.store.read_modify_write(
            self.service._path(BOARD), seed_mem, lambda: self.service._default(BOARD)
        )
        summary = self.service.archive_due_tickets(BOARD, now)
        self.assertEqual(summary["archived_memory_count"], 1)
        self.assertEqual(summary["archived_ticket_count"], 0)
        ...
        self.assertEqual(len(archived_events), 1)
        event = archived_events[0]
        self.assertEqual(event["archived_memory_count"], 1)
        self.assertEqual(event["archived_ticket_count"], 0)
        self.assertEqual(event["bytes_freed"], summary["bytes_freed"])

        counts = _health_counts(self.service)
        board_stats = counts["boards"][BOARD]
        self.assertEqual(board_stats["bytes_freed_total"], summary["bytes_freed"])
        self.assertEqual(board_stats["items_archived_total"], 1)
