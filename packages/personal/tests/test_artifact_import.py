...
def test_hashless_local_wheel_receipt_uses_exact_member_inventory(
    ...
    package_path.write_text("VALUE = 2\n", encoding="utf-8")
    with pytest.raises(
        artifacts.ArtifactVerificationError,
        match=(
            "installed fixture artifact drifted; component-lock.json is "
            "release-owned and regenerated by the operator at merge, not by a "
            "worker seat"
        ),
    ):
