...
def test_exact_view_lock_and_embedded_external_attestation_boundary() -> None:
    ...
    expected = "0ebe5bd468d4c3588f1611c084652d8243ae75ffb69ab10a1496782e83d81ca1"
    assert len(payload) == 396_499
    assert hashlib.sha256(payload).hexdigest() == expected
    assert lock["product_version"] == PRODUCT_VERSION == "5.0.0a5"
    assert lock["view"] == {
...
async def test_app_reads_leave_sqlite_domain_journal_and_cursor_unchanged(
    ...
    required = {"pursers-central": "0.1.0a11", "pursers-client": "0.1.0a10"}
