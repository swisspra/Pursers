...
def test_exact_view_lock_and_embedded_external_attestation_boundary() -> None:
    ...
    expected = "665076867fb6b1da9e3b10fa2d7d35b06d35df0a60a35a5b933d6ccfaad541db"
    assert len(payload) == 337978
    assert hashlib.sha256(payload).hexdigest() == expected
    assert lock["product_version"] == PRODUCT_VERSION == "5.0.3"
    assert lock["view"] == {
...
async def test_app_reads_leave_sqlite_domain_journal_and_cursor_unchanged(
    ...
    required = {"pursers-central": "0.1.1", "pursers-client": "0.1.2"}
    ...
    try:
        checkout_sources = {
            (Path(__file__).resolve().parents[2] / package / "src").resolve()
        ...
        try:
            sys.path[:] = [
                ...
                if Path(entry or ".").resolve() not in checkout_sources
            ]
            verified = pursers_personal.artifacts.verify_component_artifacts(
                set(required)
            )
        finally:
