...
| Path in ZIP | Source | Purpose | Added |
| --- | --- | --- | --- |
| `aion-extension.json` | `aion-extension.json` | Extension manifest | Baseline |
| `README.md` | `README.md` | Operator documentation | Baseline |
| `contexts/reviewer.md` | `contexts/reviewer.md` | Reviewer seat governance | Baseline |
| `contexts/worker.md` | `contexts/worker.md` | Worker seat governance | Baseline |
| `door/adapter.cjs` | `door/adapter.cjs` | Door onboarding adapter logic | Integration (`9e3b050`) |
| `door/DOOR_ONBOARDING_CONTRACT.md` | `door/DOOR_ONBOARDING_CONTRACT.md` | Door onboarding contract specification | Integration (`9e3b050`) |
| `security/loopback.cjs` | `security/loopback.cjs` | Loopback request validation | Integration (`9e3b050`) |
| `webui/app.js` | `webui/app.js` | Join form client logic | Baseline |
| `webui/index.html` | `webui/index.html` | Settings tab entry point | Baseline |
| `webui/routes.js` | `webui/routes.js` | API route handlers (join, status) | Baseline |
| `webui/style.css` | `webui/style.css` | Join tab styles | Baseline |
| `vendor/aion-hub-extension-schema-v0.json` | `vendor/aion-hub-extension-schema-v0.json` | Vendored JSON Schema | Baseline |
...
- **Byte-level reproducibility**: Two consecutive builds produce byte-identical archives.
...
### 2.6. No-Source-Tree Dependencies

The ZIP contains only allowlisted files. No source-tree paths, build scripts, or development dependencies are included:
...
# Step 7: Regenerate component-lock.json with pinned wheels
mkdir -p /tmp/pursers-packaging-gate/wheels
python3 tools/regenerate_component_lock.py --wheel-dir /tmp/pursers-packaging-gate/wheels
...
From a clean checkout without pre-installed packages, `verify_component_artifacts()` requires an explicit virtual environment with the pinned component distributions installed:

```sh
# 1. Build pinned wheels into an isolated directory
mkdir -p /tmp/pursers-packaging-gate/wheels
python3 tools/regenerate_component_lock.py --wheel-dir /tmp/pursers-packaging-gate/wheels

# 2. Create clean isolated virtual environment
python3.12 -m venv /tmp/pursers-packaging-gate/venv

# 3. Install built component wheels
/tmp/pursers-packaging-gate/venv/bin/pip install --disable-pip-version-check --no-input \
  /tmp/pursers-packaging-gate/wheels/pursers_central-0.1.0a29-py3-none-any.whl \
  /tmp/pursers-packaging-gate/wheels/pursers_client-0.1.0a22-py3-none-any.whl \
  /tmp/pursers-packaging-gate/wheels/pursers_personal-5.0.0a25-py3-none-any.whl

# 4. Execute component verification in clean environment
/tmp/pursers-packaging-gate/venv/bin/python -c '
...
| 2. Verify ZIP package | `python3 -m pytest -q tools/aionui-extension/tests/test_package.py` | Extension developer | 3 tests pass |
| 3. Verify manifest | `python3 -m pytest -q tools/aionui-extension/tests/test_manifest.py` | Extension developer | 3 tests pass |
| 4. Verify contexts | `python3 -m pytest -q tools/aionui-extension/tests/test_contexts.py` | Extension developer | 1 test passes |
| 5. Verify Node routes | `node --test tools/aionui-extension/tests/routes.test.cjs` | Extension developer | 6 tests pass, "pass 6, fail 0" |
| 6. Verify door adapter | `node --test tools/aionui-extension/tests/door_adapter.test.cjs` | Extension developer | 8 tests pass, "pass 8, fail 0" |
| 7. Verify team adapter | `node --test tools/aionui-extension/tests/team_adapter.test.cjs` | Extension developer | 20 tests pass, "pass 20, fail 0" |
| 8. Leak scan | `python3 tools/leak_scan.py tools/aionui-extension/` | Extension developer | clean (0 violations) |
| 9. Isolated install | Install ZIP via AionUI Settings -> Extensions -> Install from file | Operator | Settings tab renders Join form |
...
| 3. Build single-file | `cd tools/dashboard-ui && npm run build` | Dashboard developer | `tools/dashboard-ui/dashboard.html` created |
...
| 5. Digest & size check | `shasum -a 256 tools/dashboard-ui/dashboard.html packages/personal/src/pursers_personal/resources/dashboard.html` | Release engineer | Digests match exactly |
| 6. Lock regeneration | `python3 tools/regenerate_component_lock.py --wheel-dir /tmp/pursers-gate/wheels` | Release engineer | `component-lock.json` updated with matching hash |
| 7. Exact-view test | `PYTHONPATH=packages/personal/src:packages/central/src:packages/client/src python3 -m pytest -q packages/personal/tests/test_apps_contract.py -k test_exact_view_lock` | Release engineer | 1 passed |
| 8. Clean-env verify | `/tmp/pursers-gate/venv/bin/python -c "from pursers_personal.artifacts import verify_component_artifacts; verify_component_artifacts()"` | Release engineer | PASS: verified components |
| 9. No external resources | Deterministic Python assertion script in Section 3.2 | Release engineer | PASS: zero external script, stylesheet, font, CDN |

### 5.3. Fleet Dashboard Release Gate

| Step | Executable Command | Owner | Pass Condition |
| --- | --- | --- | --- |
| 1. Run tests | `python3 -m pytest -q tools/fleet-dashboard/tests/test_release_ops.py` | Fleet developer | All tests pass (21 passed) |
