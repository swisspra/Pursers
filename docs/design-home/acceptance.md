...
authenticated browser fetches the same-origin Pursers status contract in a
verifier-created CDP isolated world to obtain the running extension SHA and
reads the current selected board from rendered Home UI in that same world. It
compares both observations with the request before storing any
capture. An arbitrary 40-hex request or sandbox-looking board label therefore
cannot relabel a different runtime.
