...
The current extension manifest uses AionCore's `contributes.webui[]` contract.
It exposes `/pursers/join`, `/pursers/status`, the five
`/pursers/onboarding/*` routes, and the five `/pursers/team/*` status, plan,
apply, pause, and stop routes. Discovery therefore recognizes `door_rotation`.
The Team surface still lacks complete create/select lifecycle support, and the
seat surface lacks resume. Ticket lifecycle and result visibility are also
absent. Discovery does not award those full mutation capabilities from partial
route families, so destructive acceptance remains unavailable. When sibling
implementations land, update discovery only from their shipped interface
contract; do not invent routes or fixtures here.
...
`9e3b05072e4521c10d0f4390d7ac57ad7421c07a` ships the seven door routes. This
reviewed-input baseline additionally carries the approved Team route family,
but source discovery still finds no complete `team_lifecycle`,
`seat_lifecycle`, `ticket_lifecycle`, or `result_visibility` capability. The
corresponding mutation steps remain `SKIP` with that explicit reason rather
than inferred fixtures. Declared source routes are not proof that an installed
host executes extension handlers. This document claims no sibling contract
coverage beyond these citations.
