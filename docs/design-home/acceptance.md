...
```json
{
  ...
  "inventory": [
    {"id": "extension.join-form", "status": "passed", "evidence": "BOUNDED_REFERENCE"}
  ],
  "suites": [
    {
...
`harness.py`. Each reference must resolve to an existing regular file inside
the report directory. The validator rejects missing, escaping, oversized, or
secret-bearing artifacts and bounds both individual and aggregate bytes. Host
`version` and `build` must be nonempty exact values. Suite names and commands
must exactly match `REQUIRED_SUITES`, and every suite row must carry the same
full lowercase 40-hex reviewed candidate SHA. Verification requires the exact
sandbox target, candidate SHA, and mutation opt-in:
