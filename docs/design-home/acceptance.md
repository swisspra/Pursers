...
host identity through the verifier-owned observer. It first accepts an exact
loopback `/pursers/status` host contract; on macOS it can instead prove that the
live target listener is bundled AionCore inside the official signed and
notarized AionUi application, with matching process and bundle versions, exact
identity mode, signer team, and code-signing CDHash. A self-authored About,
helper response, receipt, or successful loopback HTTP response alone cannot
...
the installed Home entry point and reads the current selected board from the
rendered UI inside a verifier-created CDP isolated world. The package builder
generates that manifest from exact `git HEAD`.
The observer compares both values with the request before storing any capture,
so an arbitrary 40-hex request or sandbox-looking board cannot relabel another
installed runtime.
