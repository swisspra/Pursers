...
authenticated browser fetches the same-origin Pursers status contract to obtain
the running extension SHA and reads the current selected board from rendered
Home UI. It compares both observations with the request before storing any
