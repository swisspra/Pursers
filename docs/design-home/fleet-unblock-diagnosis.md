# Fleet unblock diagnosis: stale offers and Team-local waiting (resubmit-7)
...
`origin/main` `b06ce6627edb62fc588eee541fa568445b709054`.
...
Current standalone seat governance requires `bin/board.sh wait --boards pursers`
...
> `bin/board.sh wait --boards pursers --since '<FULL_CURSOR_MAP>' --timeout 300`.
> For a reviewer, re-arm
> `bin/board.sh wait --submitted --boards pursers --since '<FULL_CURSOR_MAP>' --timeout 300`.
> Do not use `--poll`, `--boards home`, or cursor `0`. Verify either one
> returned relevant event or a bounded timeout whose returned `new_seq` is a
> complete map with every selected-board cursor greater than or equal to the
> saved cursor; unchanged cursors are valid when no events occurred. Persist
> that complete successful map and re-arm once under the seat's rules. Do not
> kill or restart the seat, forget or replace doors, rejoin, change identity,
> start a duplicate wait process, or patch live state.
