# Fleet unblock diagnosis: stale offers and Team-local waiting (resubmit-4)
...
### `wait_for_boards()` behavior (project_registry.py:225-543)

1. Joins each selected board via `board_join`, caching agent_id per board
2. For each board, calls a local `drain()` function (line 305) that:
