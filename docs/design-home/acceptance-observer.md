...
| Value | Authored by |
|---|---|
| screenshot bytes, accessibility tree, observed page URL, `captured_at`, `observer_id` | observer only, from the real browser channel |
| host product / version / build | observer only, read from the live host status route |
| observation id, target, sandbox board, candidate commit, assertions | caller (runner request), verified against the observed capture |
...
- an isolated AionUi host on loopback, serving `GET /pursers/status` with the
  Pursers status contract including `host.product`, `host.version`, `host.build`
...
new one. All observations in one report must share a single `observer_id`.
...
/PATH/TO/evidence/
  ...
  host-identity.json              host_identity receipt, source host-api
...
| host status route unreachable or not the status contract | 7 | blocked, host identity unavailable |
...
Against the installed AionUi host on `http://127.0.0.1:25808`:

- `browser_channel`: ok — 50052-byte PNG, 99 accessibility nodes, observed page
  URL on the target origin, `evidence_written: false`
- `host_identity`: blocked — the status route answers `text/html; charset=utf-8`,
  not the Pursers status contract, so no capture can be recorded
- `acceptance_ready`: false

The real browser channel is therefore proven, and full acceptance stays blocked
on the supported host transport tracked by TK-23f86d56ff99. Final integrated
real-host acceptance remains a separate release gate.
