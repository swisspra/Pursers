...
| host product / version / build and running extension candidate SHA | observer only, fetched by that browser from the same-origin host status route inside a verifier-created CDP isolated world |
...
- an isolated authenticated AionUi host on loopback, serving same-origin
  `GET /pursers/status` to the browser with JSON `schema_version: 1`,
  `host.product`, `host.version`, `host.build`, and
...
- CDP `Page.createIsolatedWorld` support; host-status fetch and selected-board
  DOM reads run in that verifier world, so page-owned overrides of `fetch` or
  `Document.prototype.querySelector` cannot forge either binding
...
| authenticated host/UI binding missing or not the status contract | 7 | blocked, host, candidate, or board identity unavailable |
...
- `browser_channel`: observed a substantive PNG and accessibility tree on the
  target origin
- `host_identity`: blocked — same-origin `GET /pursers/status` answers 404, so
  the observer cannot bind AionUi identity or the running extension SHA and
  records no capture
