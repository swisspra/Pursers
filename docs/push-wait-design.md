...
**PROPOSED — cue contract:** retain `board://<board_id>/journal` as the board-wide wake cue. Add `board://<board_id>/agent/<agent_id>` as an optional precision cue for assignments or review outcomes; it supplements rather than replaces the journal URI. The per-agent URI must use the exact identity that the client joined; an omitted `agent_name` means the client's default joined identity, not a process-global worker name. A notification carries no authoritative ticket decision. On every notification or subscription reconnect, the consumer refetches committed journal state from its cursor. A normal host-timeout rotation does not refetch; it returns `timed_out=true` and re-arms from the unchanged cursor.

If Central rejects or does not honor the optional per-agent URI, the bridge retries the subscription with the journal URI alone before enabling poll compatibility mode. The board journal remains authoritative and member-authorized, so journal-only push preserves wake correctness; only the precision optimization is lost. Poll fallback is allowed only when the journal-only subscription also fails.
