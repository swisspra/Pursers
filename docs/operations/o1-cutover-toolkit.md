...
live configuration path. `validate_layout` refuses a staging root inside the
repository, a live rollback-unit member inside the repository, and relative
paths.
...
python tools/o1_cutover.py prepare --config /PATH/TO/staging/o1-cutover.json

# 3. Freeze, drain and snapshot (operator actions; the toolkit normalizes the dumps).
python tools/o1_cutover.py snapshot-boards \
  --config /PATH/TO/staging/o1-cutover.json \
  --from-json /PATH/TO/board-dump.json \
  --out /PATH/TO/staging/cutover-evidence/board-snapshot.json
python tools/o1_cutover.py snapshot-memberships \
  --config /PATH/TO/staging/o1-cutover.json \
...
python tools/o1_cutover.py backup --config /PATH/TO/staging/o1-cutover.json
python tools/o1_cutover.py verify-artifacts --config /PATH/TO/staging/o1-cutover.json

# 5. Non-mutating dry run: proves every target and every gate.
python tools/o1_cutover.py dry-run --config /PATH/TO/staging/o1-cutover.json
...
python tools/o1_cutover.py activate --config /PATH/TO/staging/o1-cutover.json \
  --operator-confirmed \
  --evidence /PATH/TO/staging/cutover-evidence/<FRESH_DRY_RUN_EVIDENCE>.json

# 7. Rollback, if any checkpoint trips.
python tools/o1_cutover.py rollback --config /PATH/TO/staging/o1-cutover.json \
  --operator-confirmed
...
| Gate | Refuses when |
| --- | --- |
| `gate_staging` | staging root missing, not `0700`, incomplete, inside the repository, config not `0600` |
...
`rollback` verifies backup integrity first, then restores in reverse journal
order (`steps_needing_rollback`), re-hashing each restored file against the
recorded pre-activation value. Afterwards `rollback_coherence_findings` proves
that the restored named credentials bind the previous resource and that their
signing keys are present in the restored JWKS. A mixed restore (old JWKS with
new tokens, or the reverse) is reported as a refusal rather than a success.
