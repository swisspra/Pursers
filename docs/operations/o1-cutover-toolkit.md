...
live configuration path. `backup_root` must be a distinct descendant of that
staging root. `validate_layout` refuses relative, absolute, traversing,
backslash-delimited, colliding or symlink-crossing rollback identifiers before
...
the backed-up JWKS before its first journal or live-file write. It then restores
in reverse journal order (`steps_needing_rollback`), re-hashing each restored
file against the recorded pre-activation value, and repeats the coherence check
on the restored live state. A mixed backup or restore (old JWKS with new tokens,
or the reverse) is refused without beginning a live restore.
