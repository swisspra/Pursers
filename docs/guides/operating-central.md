...
Unload it before a restore or uninstall:

```sh
launchctl bootout gui/$(id -u)/io.github.pursers.central
```

`KeepAlive` restarts an exited process while the job remains loaded. `bootout`
unloads the job, so it stays stopped. If you edit the plist, unload it, wait
until `launchctl print` reports that the service is not found, and then run
`bootstrap` again.
...
absolute paths. Stop Central and keep the old instance until the restore is
verified:
...
The release test took an online backup while Central was running, replaced its
data directory while stopped, restarted the LaunchAgent, and read the original
ticket successfully from the restored database.
...
On macOS, moving the environment aside is recoverable:
