...
| `agent_nudge` | Central | 5 files | 0 | None | Coordinator compatibility caller | **SUPERSEDED** | Hide a18, Remove in TK-7d99c07860bd | Superseded by autonomous Dispatcher per-seat offers, but still used by the shipped coordinator compatibility path. |
...
| `ticket_terminate` | Central | 4 files | 1 | reviewer: 1 | 2026-09-01T16:01:01Z (journal event) | **SUPERSEDED** | Hide a18, TK-7d99c07860bd | Duplicate of `ticket_cancel`; removed from Personal but retained as a deprecated Central compatibility tool. |
...
- **`agent_nudge` -> Dispatcher Offers:** The autonomous Dispatcher (TK-10da96af6455) issues targeted claim offers directly via seat queues. The shipped coordinator still references `agent_nudge` as a compatibility escape hatch; migration belongs to `TK-7d99c07860bd`. Hidden from standard tool lists in a18.
- **`ticket_assign` -> Dispatcher Matching:** Manual coordinator assignment is superseded by the automated queue dispatcher, but remains an internal/admin compatibility path until `TK-7d99c07860bd`. Hidden from standard seat tool lists in a18.
- **`ticket_terminate` -> `ticket_cancel`:** Dual verbs for marking tickets canceled/terminated created ambiguity. Personal now exposes only `ticket_cancel`; deprecated Central/client compatibility remains pending `TK-7d99c07860bd`.
...
1. **Central Tools (39):**
   ...
   - For example:
     - `agent_nudge` and `ticket_assign`: the shipped coordinator still references these deprecated compatibility paths; migration is explicitly deferred to `TK-7d99c07860bd`.
     - `ticket_terminate`: 1 call in 7 days; Personal now exposes only `ticket_cancel`, while Central/client compatibility remains deprecated pending `TK-7d99c07860bd`.
...
1. **[TK-7d99c07860bd] Migrate coordinator compatibility calls before removing `agent_nudge`**
...
2. **[TK-7d99c07860bd] Remove duplicate `ticket_terminate` in favor of `ticket_cancel`**
