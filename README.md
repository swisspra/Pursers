...
`version | 5.0.0b1` `license | Apache-2.0` `python | 3.11–3.14` `status | beta`
...
> Beta (`5.0.0b1`). One owner, one machine. Not a stable release.
...
Single-owner beta on one machine. Central binds to `127.0.0.1` loopback TLS and stores everything locally; every local process and OS user on the machine is inside its trust boundary — **not** for shared/untrusted machines, remote access, or multi-person collaboration yet.
